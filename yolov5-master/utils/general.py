...
def check_dataset(data, autodownload=True):
    ...
    train, val, test, s, force_download_script = [data.get(x) for x in ('train', 'val', 'test', 'download', 'force_download_script')]
    if val:
        val = [Path(x).resolve() for x in (val if isinstance(val, list) else [val])]  # val path
        if not all(x.exists() for x in val) or force_download_script:
